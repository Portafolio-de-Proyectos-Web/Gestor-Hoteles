...
export const TableRoom = () => {
    ...
    const deleteRoom = async (id) => {
        try {
            ...
            if (confirmDelete) {
                const { data } = await axios.delete(`http://localhost:3000/room/delete-room/${id}`)
                getRooms()
                alert(`${data.message}`)
            }
        } catch (err) {
                        ...
                        rooms.map(({ _id, numberRoom, amountPeople, typeRoom, price, status }, index) => {
                            return (
                                <tr className="text-center" key={index}>
                                    ...
                                    ></Room>
                                    <td>
                                        <Link to={`/home/rooms/updateroom/${_id}`}>
                                    ...
                                    <td>
                                        <svg onClick={() => deleteRoom(_id)} xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
