...
export const GetInvoiceDetailasPage = () => {
  ...
  const getAs = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3000/invoicedetail/getas/${_id}`
      );
  ...
  const getAsNot = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3000/invoicedetail/getasnot/${_id}`
      );
      if (data.servicesNot) {
  ...
  const addAs = async (idService) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:3000/invoicedetail/addas/${_id}`,
        form
      );
      if (data.message) {
        getAs();
  ...
  const deleteAs = async (idService) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:3000/invoicedetail/deleteas/${_id}`,
        form
      );
      if (data.message) {
        getAs();
