...
export const GetInvoiceDetailEvents = () => {
  ...
  const getEvents = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3000/invoicedetail/getevents/${_id}`
      );
      if (data.events) {
  ...
  const getEventsNot = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:3000/invoicedetail/geteventsnot/${_id}`
      );
      if (data.eventsNot) {
  ...
  const addEvent = async (idEvent) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:3000/invoicedetail/addevent/${_id}`,
        form
      );
      if (data.message) {
        getEvents();
  ...
  const deleteEvent = async (idEvent) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:3000/invoicedetail/deleteevent/${_id}`,
        form
      );
      if (data.message) {
        getEventsNot();
